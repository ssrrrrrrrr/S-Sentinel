...
TMP_DIR=".tmp/test-runtime-action-request-abort-contract"
RELEASE_ID="runtime-action-abort-request-contract-smoke"
...
cat > "$TMP_DIR/runtime-action-recommendation-$RELEASE_ID.json" <<JSON
{
  ...
  "generatedBy": "test-runtime-action-request-abort-contract.sh",
  ...
  "recommendation": {
    ...
    "summary": "Synthetic contract fixture for abort rollout request."
  },
  "runtimeSnapshot": {
...
assert binding["recommendedAction"] == "ABORT_ROLLOUT", binding
...
print("PASS runtime action request abort contract")
PY
