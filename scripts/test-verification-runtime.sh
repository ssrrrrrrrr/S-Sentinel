...
EXTERNAL_UNAVAILABLE="$TMP_DIR/verification-external-command-unavailable.json"
...
echo "===== external-command runtime explicitly enabled but verifier unavailable ====="
S_SENTINEL_VERIFICATION_ALLOW_EXTERNAL_COMMAND=1 \
scripts/verification-runtime.py external-command-preview \
  --supply-chain-decision "$SUPPLY_CHAIN" \
  --cosign-bin /tmp/ssentinel-missing-cosign \
  --output "$EXTERNAL_UNAVAILABLE"
...
"$PYTHON_BIN" - "$INPUT_DERIVED" "$EXTERNAL_PREVIEW" "$EXTERNAL_UNAVAILABLE" "$EXTERNAL_ENABLED" "$ADMISSION_PLACEHOLDER" "$FAKE_COSIGN" <<'PY'
import json
import sys
...
external_unavailable = json.loads(Path(sys.argv[3]).read_text(encoding="utf-8"))
external_enabled = json.loads(Path(sys.argv[4]).read_text(encoding="utf-8"))
admission = json.loads(Path(sys.argv[5]).read_text(encoding="utf-8"))
fake_cosign = sys.argv[6]

for item in [input_derived, external, external_unavailable, external_enabled, admission]:
...
assert input_derived["verificationStatus"] == "input_derived", input_derived
...
assert external["verificationStatus"] == "external_command_disabled", external
...
assert external_unavailable["mode"] == "external_command", external_unavailable
assert external_unavailable["verificationStatus"] == "external_tool_unavailable", external_unavailable
assert external_unavailable["toolAvailable"] is False, external_unavailable
assert external_unavailable["command"] is None, external_unavailable
assert external_unavailable["exitCode"] is None, external_unavailable
assert external_unavailable["results"]["externalVerificationRequested"] is True, external_unavailable
assert external_unavailable["results"]["externalVerificationAllowed"] is True, external_unavailable
assert external_unavailable["results"]["externalVerificationExecuted"] is False, external_unavailable
assert external_unavailable["results"]["externalVerificationSkippedReason"] == "tool_not_available", external_unavailable
assert external_unavailable["guardrails"]["doesNotRunExternalCommands"] is True, external_unavailable

assert external_enabled["mode"] == "external_command", external_enabled
assert external_enabled["verificationStatus"] == "external_verification_passed", external_enabled
...
assert admission["verificationStatus"] == "admission_placeholder", admission
