...
def default_human_steps(release_result: str, final_action: str, requires_approval: bool) -> list[str]:
    steps = [
        "Confirm the stable version is still serving traffic normally.",
        "Inspect canary logs, Kubernetes events, AnalysisRun metrics, and the related change context.",
        "Verify whether SLO metrics have recovered below their configured thresholds.",
    ]

    if final_action in {"STOP_PROMOTION", "ABORT_ROLLOUT"}:
        steps.append("Do not increase canary traffic before human review is completed.")

    if requires_approval:
        steps.append("Human approval is required before executing any write operation.")

    if release_result == "PASS":
        return ["Release passed. Archive the release record and continue observing key metrics."]
