...
TEST_TMP=".tmp/test-runtime-action-execution-result-mock-abort"
...
bash scripts/test-runtime-action-execution-result-mock-abort.sh >/tmp/ssentinel-runtime-action-execution-result-abort-record-source.log
tail -40 /tmp/ssentinel-runtime-action-execution-result-abort-record-source.log
...
cat > "$EVIDENCE_PATH" <<JSON
{
  ...
  "service": "demo-app",
  "env": "dev",
  "version": "v-runtime-action-execution-result-abort-evidence",
  "generatedBy": "test-evidence-record-runtime-action-execution-result-abort-links.sh",
...
release_id = "runtime-action-abort-execution-mock-smoke"
record = json.loads(Path(f".tmp/test-runtime-action-execution-result-mock-abort/evidence-record-{release_id}.json").read_text(encoding="utf-8"))
...
assert execution["requestedAction"] == "ABORT_ROLLOUT", execution
...
assert execution["abortVerified"] is True, execution
assert execution["didPause"] is False, execution
assert execution["didResume"] is False, execution
assert execution["didAbort"] is True, execution
...
print("PASS evidence record runtime action execution result abort links")
PY
