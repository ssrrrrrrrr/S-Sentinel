...
TMP_DIR=".tmp/test-runtime-action-preflight-rollback-contract"
RELEASE_ID="runtime-action-rollback-preflight-contract-smoke"
...
cat > "$TMP_DIR/runtime-action-recommendation-$RELEASE_ID.json" <<JSON
{
  ...
  "generatedBy": "test-runtime-action-preflight-rollback-contract.sh",
  ...
  "recommendation": {
    ...
    "reasons": ["manual_rollback_contract_fixture"],
    "summary": "Synthetic contract fixture for rollback rollout preflight."
  },
  "runtimeSnapshot": {
...
assert doc["request"]["requestedAction"] == "ROLLBACK_ROLLOUT", doc
...
assert "rollback_runtime_action_contract_only" not in preflight["blockingReasons"], preflight
...
assert gate["operationGateEnv"] == "S_SENTINEL_ALLOW_RUNTIME_ROLLBACK", gate
...
print("PASS runtime action preflight rollback contract")
PY
