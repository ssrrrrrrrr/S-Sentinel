...
TMP_DIR=".tmp/test-runtime-action-request-rollback-contract"
RELEASE_ID="runtime-action-rollback-request-contract-smoke"
...
cat > "$TMP_DIR/runtime-action-recommendation-$RELEASE_ID.json" <<JSON
{
  ...
  "generatedBy": "test-runtime-action-request-rollback-contract.sh",
  ...
  "recommendation": {
    ...
    "summary": "Synthetic contract fixture for rollback rollout request."
  },
  "runtimeSnapshot": {
...
assert binding["recommendedAction"] == "ROLLBACK_ROLLOUT", binding
...
print("PASS runtime action request rollback contract")
PY
