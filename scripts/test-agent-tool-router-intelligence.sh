...
TEST_TMP="${1:-/tmp/slo-agent-tool-router-intelligence-test}"
rm -rf "$TEST_TMP"
mkdir -p "$TEST_TMP"

echo
echo "===== run agent tool router release intelligence test ====="
...
RELEASE_REPORT_DIR="$TEST_TMP" \
  ./scripts/build-release-memory.sh >"$TEST_TMP/memory.log" 2>&1

RELEASE_REPORT_DIR="$TEST_TMP" \
RELEASE_INTELLIGENCE_OUTPUT_DIR="$TEST_TMP" \
  ./scripts/agent-tool-router.sh build-release-intelligence "$TEST_TMP/release-evidence-current.json" \
  >"$TEST_TMP/router.log" 2>&1

cat "$TEST_TMP/router.log"

grep -q "toolName: build-release-intelligence" "$TEST_TMP/router.log"
grep -q "readOnlyAnalysis: true" "$TEST_TMP/router.log"
grep -q "Release intelligence JSON generated" "$TEST_TMP/router.log"
...
assert data["guardrails"]["readOnlyAnalysis"] is True, data["guardrails"]
...
print("Agent tool router release intelligence test passed")
PY
