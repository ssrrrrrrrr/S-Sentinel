...
TMP_DIR=".tmp/test-runtime-action-preflight-promote-contract"
RELEASE_ID="runtime-action-promote-preflight-contract-smoke"
...
cat > "$TMP_DIR/runtime-action-recommendation-$RELEASE_ID.json" <<JSON
{
  ...
  "generatedBy": "test-runtime-action-preflight-promote-contract.sh",
  ...
  "recommendation": {
    ...
    "summary": "Synthetic contract fixture for promote rollout preflight."
  },
  "runtimeSnapshot": {
...
assert preflight["preflightStatus"] == "BLOCKED", preflight
...
assert "promote_runtime_action_contract_only" in preflight["blockingReasons"], preflight
...
assert gate["operationGateEnv"] is None, gate
...
print("PASS runtime action preflight promote contract")
PY
