...
func TestRuntimeActionRecommendationPortalResourceMapping(t *testing.T) {
	const kind = "runtimeActionRecommendation"
	const segment = "runtime-action-recommendation"
	const prefix = "runtime-action-recommendation"
	...
	fileName := prefix + "-20260527-020202.json"
	if got := kindFromReportFile(fileName); got != kind {
		t.Fatalf("kindFromReportFile(%q) = %q, want %q", fileName, got, kind)
	}
	if got := releaseIDFromReportFile(fileName); got != "20260527-020202" {
		t.Fatalf("releaseIDFromReportFile(%q) = %q, want 20260527-020202", fileName, got)
	}
