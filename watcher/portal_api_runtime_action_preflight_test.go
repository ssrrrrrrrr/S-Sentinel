...
func TestRuntimeActionPreflightPortalResourceMapping(t *testing.T) {
	const kind = "runtimeActionPreflight"
	const segment = "runtime-action-preflight"
	const prefix = "runtime-action-preflight"
	...
	fileName := prefix + "-20260527-040404.json"
	if got := kindFromReportFile(fileName); got != kind {
		t.Fatalf("kindFromReportFile(%q) = %q, want %q", fileName, got, kind)
	}
	if got := releaseIDFromReportFile(fileName); got != "20260527-040404" {
		t.Fatalf("releaseIDFromReportFile(%q) = %q, want 20260527-040404", fileName, got)
	}
