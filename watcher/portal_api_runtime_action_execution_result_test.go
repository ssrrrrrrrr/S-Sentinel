...
func TestRuntimeActionExecutionResultPortalResourceMapping(t *testing.T) {
	const kind = "runtimeActionExecutionResult"
	const segment = "runtime-action-execution-result"
	const prefix = "runtime-action-execution-result"
	...
	fileName := prefix + "-20260527-050505.json"
	if got := kindFromReportFile(fileName); got != kind {
		t.Fatalf("kindFromReportFile(%q) = %q, want %q", fileName, got, kind)
	}
	if got := releaseIDFromReportFile(fileName); got != "20260527-050505" {
		t.Fatalf("releaseIDFromReportFile(%q) = %q, want 20260527-050505", fileName, got)
	}
