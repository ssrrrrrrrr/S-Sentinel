...
func TestRuntimeActionRequestPortalResourceMapping(t *testing.T) {
	const kind = "runtimeActionRequest"
	const segment = "runtime-action-request"
	const prefix = "runtime-action-request"
	...
	fileName := prefix + "-20260527-030303.json"
	if got := kindFromReportFile(fileName); got != kind {
		t.Fatalf("kindFromReportFile(%q) = %q, want %q", fileName, got, kind)
	}
	if got := releaseIDFromReportFile(fileName); got != "20260527-030303" {
		t.Fatalf("releaseIDFromReportFile(%q) = %q, want 20260527-030303", fileName, got)
	}
