...
function TimelineItem({ event }: { event: TimelineEvent }) {
  ...
  return (
    <div className="relative grid gap-4 rounded-xl border border-[#1f2b3d] bg-[#0b121d] p-4 shadow-sm md:grid-cols-[240px_minmax(0,1fr)]">
      ...
      <div className="grid gap-2 text-sm">
        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 rounded-lg bg-[#070b12] px-3 py-2">
          <span className="text-slate-500">状态</span>
          <span className={collected ? "font-semibold text-emerald-300" : "font-semibold text-amber-300"}>
            {status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 rounded-lg bg-[#070b12] px-3 py-2">
          ...
          <span className="break-words font-mono text-slate-100">{event.artifactKind ?? "-"}</span>
        </div>

        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 rounded-lg bg-[#070b12] px-3 py-2">
          <span className="text-slate-500">Artifact</span>
          <span className="break-words font-mono text-slate-100">{event.artifact?.baseName ?? "-"}</span>
        </div>

        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 rounded-lg bg-[#070b12] px-3 py-2">
          <span className="text-slate-500">ModifiedAt</span>
          <span className="break-words font-mono text-slate-100">{formatTime(event.artifact?.modifiedAt)}</span>
        </div>

        <div className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 rounded-lg bg-[#070b12] px-3 py-2">
          <span className="text-slate-500">Size</span>
          <span className="font-mono text-slate-100">{formatBytes(event.artifact?.sizeBytes)}</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-sky-100 bg-sky-50 p-4">
        ...
        <div className="mt-3 inline-flex rounded-full border border-sky-200 bg-[#0b121d] px-3 py-1 text-xs font-semibold text-sky-800">
          Source：{sourceMode}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-emerald-900/45 bg-emerald-950/20 p-4 text-sm leading-6 text-emerald-200">
